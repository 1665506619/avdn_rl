...
class ET(nn.Module):
    def __init__(self, args):
        ...
        self.decoder_2_action_full = nn.Linear(64, 4)
        self.stop_head = nn.Linear(64, 1)
        self.value_head = nn.Linear(64, 1)
        init_std = getattr(args, "ppo_action_std_init", 0.2)
        self.action_log_std = nn.Parameter(torch.log(torch.full((3,), init_std)))
    ...
    def forward(self, **inputs):
        ...
        policy_feature = self.policy_feature(action_decoder_input)
        action = self.decoder_2_action_full(policy_feature)
        stop_logit = self.stop_head(policy_feature).squeeze(-1)
        state_value = self.value_head(policy_feature).squeeze(-1)
    ...
    def init_weights(self, init_range=0.1):
        ...
        self.decoder_2_action_full.bias.data.zero_()
        self.decoder_2_action_full.weight.data.uniform_(-init_range, init_range)
        self.stop_head.bias.data.zero_()
        self.stop_head.weight.data.uniform_(-init_range, init_range)
        self.value_head.bias.data.zero_()
        self.value_head.weight.data.uniform_(-init_range, init_range)
