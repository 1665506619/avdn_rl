...
def generate_attention_mask(len_lang, len_frames, device, num_input_actions=0):
    ...
    frames_to_all = torch.cat((frames_to_lang, frames_to_frames, frames_to_directions), dim=1)
...
def extract_action_preds_list(m_out_list, pad, vocab_action, clean_special_tokens=True, lang_only=False):
    m_out_action = m_out_list[0]["action"] + m_out_list[1]["action"] + m_out_list[2]["action"]
    m_out_object = m_out_list[0]["object"] + m_out_list[1]["object"] + m_out_list[2]["object"]
