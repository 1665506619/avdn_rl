...
if [[ "${DEBUG_CUDA}" == "1" ]]; then
  ...
  echo "Debug mode enabled: CUDA_LAUNCH_BLOCKING=1 TORCH_DISTRIBUTED_DEBUG=DETAIL NCCL_DEBUG=WARN GLOO_SOCKET_IFNAME=${GLOO_SOCKET_IFNAME} NCCL_SOCKET_IFNAME=${NCCL_SOCKET_IFNAME}"
fi
