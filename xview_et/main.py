...
def train(args, train_env, train_full_traj_env, val_envs, val_full_traj_envs, aug_env=None, rank=-1):
    ...
    def log_agent_losses(agent_obj, step, header):
        tracked = [
            'IL_loss', 'position_loss', 'angle_loss', 'altitude_loss',
            'progress_loss', 'stop_loss', 'stop_acc', 'nss_loss',
            'ppo_loss', 'ppo_policy_loss', 'ppo_value_loss',
            'ppo_entropy', 'ppo_ratio', 'ppo_rollout_return',
            'ppo_rollout_len', 'ppo_il_anchor',
        ]
        parts = [header]
        for key in tracked:
            if key in agent_obj.logs and len(agent_obj.logs[key]) > 0:
                value = sum(agent_obj.logs[key]) / max(len(agent_obj.logs[key]), 1)
                if writer is not None:
    ...
    for idx in range(start_iter, start_iter+args.iters, interval):
        ...
        if default_gpu:
            write_to_record_file('\n' + log_agent_losses(agent, iter, 'IL stage'), record_file)
        ...
        if default_gpu:
            ...
            eval_args.local_rank = -1
            agent_eval = agent_class_eval(eval_args, rank=0)
        ...
        if default_gpu:
            for env_name, env in val_envs.items():
                ...
                loader = torch.utils.data.DataLoader(env, batch_size = 1)
                agent_eval.test(loader, feedback='teacher')
    ...
    if args.use_ppo and args.ppo_iters > 0:
        agent.logs = defaultdict(list)
        if args.train_val_on_full:
            agent.env = train_full_traj_env
        else:
        ...
        if default_gpu:
            ...
            agent.save(ppo_step, os.path.join(args.ckpt_dir, "latest_ppo"))

            eval_args = copy.deepcopy(args)
            eval_args.world_size = 1
            eval_args.local_rank = -1
            agent_eval = agent_class(eval_args, rank=0)
            agent_eval.load(os.path.join(args.ckpt_dir, "latest_ppo"))
            ppo_loss_str = "ppo_iter {}".format(ppo_step)
            for env_name, env in val_envs.items():
                agent_eval.env = env
                loader = torch.utils.data.DataLoader(env, batch_size=1)
                agent_eval.test(loader, feedback='student')
                pred_results = agent_eval.get_results()
                score_summary, _ = env.eval_metrics(pred_results)
                ppo_loss_str += ", %s " % env_name
                for metric, val in score_summary.items():
                    ppo_loss_str += ', %s: %.2f' % (metric, val)
                    if writer is not None:
